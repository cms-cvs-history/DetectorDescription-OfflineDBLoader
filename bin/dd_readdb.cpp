...
int  main( int argc, char** argv )
{
  ...
  opt.helpEntry = "the source XML configuration file name for SpecPars";
  opt.required = true;
  ops.push_back(opt);
  opt.flag = "-c";
  ...
  try{
	...
	cout << "Environment variable DDWriteConnectString was used for connection. ";
      ...
      ReadWriteORA rwo( theConnectionString
			...
			, theUserName
			, thePassword );
      bool result = rwo.readFromDB();
      if ( !result ) {
	cout << "Failed to read DB Geometry. " << endl;
      } else {
	result = rwo.readFromXML();
	if ( !result ) {
	  cout << "Failed to read XML (should contain only SpecPars)." << endl;
	}
      }
